#' @export
pxciarFun <- function(x=pxzozoned[1,irregpcode(substr(rc,1,3))],nn='pxzozoned') {
  # - [ ] define area
  getgd(nn)
  pxciard <<- regpcode(x)
  putt(pxciard)
...
#' @export
pxcisu2Fun <- function(nn='pxciard') {
...
#' @export
pxcicpxFun <- function(nn=c('pxjocpd','pxciard')) {
  # - [ ] subset cpd to area
  getgd(nn)
...
#' @export
pxcirdo4Fun <- function(nn=c('pxg2rdo4d','pxcisu2d')) {
...
#' @export
pxcifadeFun <- function(nn=c('pxcisu2d','pxsofaded')) {
  # - [ ] pxcifade (err)
  getgd(nn)
...
#' @export
pxcippm1Fun <- function(nn=c('pxpvppm1d','pxcicpxd','pxcisu2d')) {
  # - [ ] pxcippm1 (ppm)
  getgd(nn)
...
#' @export
pxcierrFun <- function(nn=c('pxcifaded','pxcisu2d','pxcicpxd'),yearsmax=22,daysmin=90,errmax=1,errmin=-1) {
  # - [ ] screen err
  getgd(nn)
...
#' @export
pxcippmFun <- function(nn=c('pxcippm1d'),pvmax=50e6,pvmin=1e4,m2max=500,m2min=30,ppm2max=2e4,ppm2min =2e2) {
  # - [ ]  screen ppm
  getgd(nn)
...
#' @export
pxcicercFun <- function(nn=c('pxcippmd','pxcierrd'),k1=1) {
...
#' @export
pxcicenFun <- function(nn=c('pxcicercd','pxcippmd','pxcierrd')) {
...
#' @export
pxcinbrFun <- function(ty=c('pxcippmd','pxcierrd'),nn='pxcicend',k1=getpv('cinbr','k1br'),...) {
...
#' @export
pxcinoupFun <- function(nn=c('pxcippmd','pxcinbrd'),k1=getpv('cinoup','k1p'),p1=getpv('cinoup','p1p'),...) {
...
#' @export
pxcinoueFun <- function(nn=c('pxcippmd','pxcierrd','pxcinbrd'),k1=getpv('cinoue','k1e'),p1=getpv('cinoue','p1e'),...) {
  # - [ ] rnk, p, weight, inlier
  getgd(nn)
  cinoue <- function(...) {
    cinou(x='pxcierrd',cinbrd=pxcinbrd,jrank='err',...)
  }
  pxcinoued <<- cinoue(k=k1,p=p1)
  putt(pxcinoued)
}
pxcinouechk <- function(nn=c('pxcinoued','pxcinoupd','pxcicend','pxcinbrd')) {
...
#' @export
pxcinouaFun <- function(yrs=2^(0:4),nn=c('pxcinoued','pxlrsetdad')) {
...
#' @export
pxcivoFun <- function(nn=c('pxcicpxd','pxcicend','pxciard','pxosrdo1d'),
...
#' @export
pxciagFun <- function(nn=c('pxcinoupd','pxcinoued','pxcicntnd'),startyear=1995) {
...
#' @export
pxcijoFun <- function(nn=c('pxciagd','pxcivod')) {
...
#decode step
#' @export
pxcidecnFun <- function(typex=c('D','S','T','F'),nn='pxcicntnd') {
...
#encode step
#' @export
pxcicntnFun <- function(nn='pxcinoupd',brkx=brk(),countthresh=c(0,3),typex=c('F','D','S','T')) {
...
pxcicntnchk <- function(nn=c('pxcinoupd','pxcicntnd','pxcidecnd','pxciagd'),jx=NULL) { #incomplete, just check on random sample that something is in the bin when flagged
  getgd(nn)
